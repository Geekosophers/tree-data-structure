class BinarySearchTree {
  ...
  insert(data) {
    ...
    if (width > height) length = width / 20;
    else length = height / 30;

    if (this.root === null) {
      newNode.coord = coord;
      newNode.parentCoord = coord;
      newNode.level = 0;
      newNode.circle = new Circle(
        newNode.coord[0],
        newNode.coord[1],
        newNode.data.toString(),
        length
      );
  ...
  insertNode(node, newNode, length) {
    if (newNode.data < node.data) {
      if (node.left === null) {
        ...
        newNode.coord = [
          node.coord[0] - (3 / newNode.level) * length,
          node.coord[1] + length
        ];
        newNode.circle = new Circle(
          newNode.coord[0],
        ...
        node.left = newNode;
        return node.left;
      } else return this.insertNode(node.left, newNode, length);
    } else {
      if (node.right === null) {
        newNode.level = node.level + 1;
        newNode.parentCoord = node.coord;
        ...
        node.right = newNode;
        return node.right;
      } else return this.insertNode(node.right, newNode, length);
    }
  }
